...
#define HOSTAP_TASKLET_INIT(q, f) \
do { memset((q), 0, sizeof(*(q))); (q)->func = (f); } \
while (0)
	...
	HOSTAP_TASKLET_INIT(&local->rx_tasklet, hostap_rx_tasklet);
	skb_queue_head_init(&local->rx_list);

	HOSTAP_TASKLET_INIT(&local->sta_tx_exc_tasklet, hostap_sta_tx_exc_tasklet);
